...
void print_node(const CWBVHNode node) {
	printf("P: %f, %f, %f\n", node.pOrigin.x, node.pOrigin.y, node.pOrigin.z);
	printf("Exponent: %d, %d, %d\n", node.e.x, node.e.y, node.e.z);
	printf("imask: 0x%x\n", node.imask);
	printf("Node base: 0x%x \tTri base: 0x%x\n", node.nodeBaseIndex, node.triBaseIndex);
	
	for (int i=0; i<8; i++) {
		printf("Child %d meta: %d %d\n", i, node.childMetaData[i].upper, node.childMetaData[i].lower);
		printf("Child %d: (%d, %d, %d) - (%d, %d, %d)\n", i, node.qlo[i].x, node.qlo[i].y, node.qlo[i].z, node.qhi[i].x, node.qhi[i].y, node.qhi[i].z);
	}
}
...
)
{
	...
	int stackPtr = 0;
	std::unordered_map<int, int> tree_depth_map; 
	...
	while (stackPtr > 0)
	{
		...
		if (node->ChildCount != 0)
		{
			...
			tree_depth_map[currentNodeAddr] = tree_depth_map[parentNodeAddr] + 1;
		...
		else
		{
			...
			tree_depth_map[currentNodeAddr] = tree_depth_map[parentNodeAddr] + 1;
			max_tree_depth = std::max(max_tree_depth, tree_depth_map[currentNodeAddr]);
	...
	std::cout << "Max tree depth: " << max_tree_depth << std::endl;
}
...
)
{
	...
	while (stackPtr > 0)
	{
		...
		// printf("Child Index: 0x%x \tTri Index: 0x%x\n", childBaseIndex, triangleBaseIndex);
	}
}
...
)
{
	...
	CWBVHNode currentNode;
	OutIntermediates.BVHNodeData.push_back(currentNode);
	...
	while (stackPtr > 0) {
		...
		unsigned char imask = 0;
		unsigned int childBaseIndex = 0;
		unsigned int triangleBaseIndex = 0;

		for (int childIndex = 0; childIndex < 8; childIndex++)
		{
			if (node->Children[childIndex] == nullptr) {
				currentNode.childMetaData[childIndex].upper = 0;
				currentNode.childMetaData[childIndex].lower = 0;
				continue;
			}
			...
			currentNode.qlo[childIndex].x = (unsigned char)qlo.x;
			currentNode.qlo[childIndex].y = (unsigned char)qlo.y;
			currentNode.qlo[childIndex].z = (unsigned char)qlo.z;
			currentNode.qhi[childIndex].x = (unsigned char)qhi.x;
			currentNode.qhi[childIndex].y = (unsigned char)qhi.y;
			currentNode.qhi[childIndex].z = (unsigned char)qhi.z;
			...
			if (node->Children[childIndex]->LeafPrimitiveCount == 0)
			{
				...
				// Preallocate space for child
				CWBVHNode nextNode;
				OutIntermediates.BVHNodeData.push_back(nextNode);
				...
				currentNode.childMetaData[childIndex].upper = 1;
				currentNode.childMetaData[childIndex].lower = 24 + childIndex;
			...
			else
			{
				...
				currentNode.childMetaData[childIndex].upper = unaryEncodedPrimitiveCount;
				currentNode.childMetaData[childIndex].lower = leafChildPrimitiveCount;
		...
		currentNode.pOrigin = nodeLo;
		currentNode.nodeBaseIndex = childBaseIndex;
		currentNode.triBaseIndex = triangleBaseIndex;
		currentNode.imask = imask;
		
		OutIntermediates.BVHNodeData[currentNodeAddr] = currentNode;
		#ifdef DEBUG
